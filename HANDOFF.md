...
- `npm.cmd run lint`：通过。
- `npm.cmd test`：当前 506/509；失败项为既有的 `scripts/chatLongTermRecallBudget.test.ts`、`scripts/cinemaApp.test.ts` 和 `scripts/memoryCrossAppScope.test.ts` 静态契约，与角色手机改动无关。
- `npm.cmd run build`：通过；只有既有的大 chunk 性能提示，不是失败。
- `git diff --check`：通过；CRLF 提示是 Git 的换行提示，不是代码错误。

本轮关系网 NPC 增量的定向验证：`npx.cmd tsx scripts/characterPhoneRelationshipNetwork.test.ts`、`npx.cmd tsx scripts/characterPhoneContent.test.ts`、`npm.cmd run lint` 均通过。右侧浏览器重新加载最新代码后，选择“步随影”、输入角色手机密码 `0000`，确认聊天列表出现关系网联系人“旧识”；点击桌面生成图标完成真实 API 生成后，该联系人出现独立的来信/角色回复，且没有写入用户与角色的聊天镜像。朋友圈页面继续只展示已有角色/用户/可见联系人动态；本次关系网 NPC 没有对应主手机 Moment，因此没有凭空追加 NPC 动态。

右侧浏览器逐页验收通过：使用已配置的第三方 API 完成自然聊天后，选择“步随影”、输入密码 `0000`、点击桌面生成图标，确认返回并持久化了新的生活痕迹。桌面会展示“最近生活轨迹”，并可跳转到对应应用；逐页检查了聊天（用户镜像）、通讯录（仅有依据的用户联系人）、朋友圈（角色动态）、浏览器（API 额度搜索记录）、日记、备忘录、日程、相册、音乐、通话、相机和设置。相机支持本地真实图片导入/移动端拍摄入口并保存到角色相册，不把图片发送给生成 API；没有足够证据的日程、相册、音乐、通话保持空状态，没有用模板填充。重复生成仍能返回，生成按钮会恢复可用。
...
- 新增角色手机内容与 UI 回归断言；角色手机定向测试、lint、构建和 `git diff --check` 均通过。当前全量测试为 506/509，失败项为既有的 `scripts/chatLongTermRecallBudget.test.ts`、`scripts/cinemaApp.test.ts` 和 `scripts/memoryCrossAppScope.test.ts` 静态契约，与角色手机改动无关；右侧浏览器已复验。
...
- 隐藏相册新增独立密码门禁；当前验收密码为 `3737`。密码解析通过 `hiddenGalleryPasscode` 预留按角色覆盖入口，后续可接入人设/世界书派生规则。
