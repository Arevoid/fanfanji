...
## 17. Round 8 AI 阅读游标与知识边界

- 每个房间的 `AiReadingState` 现在记录正常阅读游标、已知章节、每章已知段落范围、阅读速度、主动评论频率和剧透策略。
- `advanceAiReadingToParagraph` 只接受当前身份、当前书籍的真实段落锚点，并且单调推进；跨身份、跨书籍或不存在的锚点会被拒绝，已知范围不会回退。
- AI 上下文投影只返回已知段落。未来段落进入 `blockedAnchorIds`，不会因为本地存在解析数据或 Book Bible 就自动可见。
- 用户明确召唤并分享未来片段时，只保存不超过 8000 字的冻结 `userRevealedSpoiler`，该记录只属于当前房间，且不会推进 AI 的正常阅读游标或把整章标记为已读。
- 房间详情允许用户明确点击“让 TA 读到我的当前位置”；不会自动把私人进度、笔记或其他房间内容分享给 AI。
- 这一轮仍不调用模型。后续段评/召唤 Prompt 必须只使用 `buildAiReadingContext` 返回的已知片段与显式剧透片段。

## 18. Round 9 共读评价与召唤讨论

- 共读存储新增 `ReadingComment`、`ReadingDiscussion` 和 `ReadingDiscussionMessage`，全部带完整房间作用域。
- 评论支持段评、章评、全书评价和回复类型；私人阅读中的高亮、笔记不会自动转为共读评论。
- AI 段评必须携带目标段落和文本冻结快照，并通过 `buildAiReadingContext` 校验；未读段落或未明确分享的段落会被拒绝。
- “召唤 TA”会在当前房间创建 `pending_ai` 讨论线程和用户首条消息，后续由 AI 生成服务追加结构化回复，不跳转真人聊天。
- 评论与讨论查询都按完整房间作用域过滤；替换房间 ID、关系 ID 或会话 ID 后无法读到原线程。
- 当前 UI 已提供房间内全书评价输入、评论列表和召唤讨论输入；AI 回复、段评锚点选择与记忆确认留待后续轮次。
