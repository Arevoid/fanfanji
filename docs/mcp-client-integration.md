...
若本机已有「联网」配置，设置页会自动把旧显示名 `Research MCP · 联网研究 MCP` 迁移为「联网」。同时会加入「热搜」，改为使用米饭机自己的同源地址 `/api/hotsearch-mcp`，不再依赖不稳定的公共端点。它提供 `list_platforms` 和 `query_hot_trending` 两个只读工具，聚合微博、知乎、B站、百度、头条、抖音、贴吧、掘金热榜；结果缓存 5 分钟，单个平台失败时不会影响其他平台。
