...
角色手机内容生成必须保持以下边界：联系人只能来自用户、已有角色关系或有明确上下文依据的 NPC；NPC 对话写入独立联系人线程，不能塞进角色与用户的聊天镜像；角色朋友圈只包含角色本人、用户和角色认识的联系人可见动态；用户不认识角色的好友不得被展示；文件名、世界书标题和字段名不能被当作角色姓名或正文；首次初始化按批量基线生成聊天 4—6 条、浏览器 4—8 条、未来 3—6 天日程 1—3 条、日记/备忘录/待办合计 2—4 条、至少一条仅用户/仅自己可见朋友圈和音乐收听/常听时段/当前播放数据，后续追加随机选择少量应用；没有足够依据时保持空，不使用模板填充。主手机音乐库同步不能覆盖角色专属生成曲目及其收听历史。
...
本次角色手机内容生成修复已通过定向测试、`npm.cmd run lint`、`npm.cmd run build` 和 `git diff --check`；全量 `npm.cmd test` 当前为 526/526。首次批量生成、3—6 天日程窗口、音乐生成数据和音乐跨同步保留均有回归断言。本轮没有进行右侧浏览器真实 API 验收，不把自动化测试或生产构建冒充真实移动设备、staging 或弱网验收。
...
- 隐藏相册需要单独输入密码；新角色手机会根据角色资料派生并在创建时先持久化独立密码，不再使用全局测试密码。旧记录若没有独立 `hiddenGalleryPasscode`，兼容使用该角色手机已保存的解锁密码，避免锁死已有数据。
...
- 角色手机导航栏统一增量：以用户手机常规应用顶部栏实测 64px 为基准，普通应用顶部栏、聊天子页、浏览器搜索详情和相册图片详情统一使用 `box-border h-16 min-h-16 max-h-16`；浏览器首页返回区域也固定 64px，外层返回/Today/保存控件统一为 32px 命中区。
- 浏览器首页返回区域与日程共用 16px 横向内边距和 1.5 间距，返回按钮左缘保持同一条对齐线。
- 浏览器首页导航改为正常文档流占位，Google 标志与搜索区从 64px 导航栏下方开始渲染，不再与返回按钮同一行重叠。
- 日程右上角操作改为小号 `Today`（不再强制大写），点击仍通过今天信号将月份和选中日期复位到系统当天。
- 浏览器首页外层背景统一为纯白，消除状态栏下方浅灰外层与 Google 内容区之间的横向色带。
- 日程外层背景与正文统一为纯白，并移除正文顶部重置产生的额外 16px 空白，避免导航栏视觉上显得偏高。
