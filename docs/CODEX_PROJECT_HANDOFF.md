...
历史记录不能代替当前修改后的验证。每次代码修改都应重新运行与风险匹配的检查；提交前优先运行 `npm run check`。构建中出现 AppChat、`vendor-docx` 等大 chunk 属于性能提示，不等于质量门禁失败。
...
- 角色手机实验改动与普通 bug 修复分离；在用户未重新授权前，不修改、不验证、不推送。
