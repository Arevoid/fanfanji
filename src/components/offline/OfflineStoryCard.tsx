    ...
    <article className={`offline-story-card ${isUser ? "is-user" : "is-character"}`}>
      <header className="offline-story-card-header">
        ...
        <div className="offline-node-menu-anchor">
          <button type="button" className="offline-node-trigger" onClick={onMenuToggle} aria-label="打开剧情节点菜单"><MoreHorizontal size={20} /></button>
          {menuOpen && <OfflineNodeMenu onEdit={onEdit} onDelete={onDelete} onGuidance={onGuidance} onClose={onMenuToggle} />}
        </div>
      </header>
