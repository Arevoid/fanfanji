...
export default function ReadingStoryPlayShell(
  props: ReadingStoryPlayShellProps,
) {
    ...
    >
      <header className="relative z-30 flex shrink-0 items-center justify-between border-b border-white/10 bg-[#0c0b0a]/95 px-3 py-2 backdrop-blur">
        <button
          ...
          aria-label="退出故事"
          className="flex h-9 w-9 items-center justify-center rounded-full text-[#eee8df]"
        >
      ...
      <div className="relative z-20 h-1 shrink-0 bg-white/5">
        <div
      ...
      >
        <div className="mx-auto max-w-2xl">
          <div className="mb-5 flex flex-wrap items-center gap-2 text-[10px] text-white/45">
            ...
            <span className="ml-auto text-amber-300/75">
              {props.statusLabel}
            </span>
          </div>
          {props.notice && (
            <div
              role="status"
              className="mb-5 rounded-2xl border border-amber-300/20 bg-amber-400/10 px-4 py-3 text-xs leading-5 text-amber-50/80"
            >
              {props.notice}
            </div>
          )}
        ...
        <div className="mx-auto max-w-2xl">
                ...
                <div className="mt-2 max-h-48 space-y-2 overflow-y-auto pr-1">
                  {props.choices.map((choice, index) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        props.onActionChange(choice.label);
                    ...
                    >
                      ...
                      {choice.consequenceHint && (
                        <span className="mt-1 block pl-6 text-[10px] text-white/40">
                          {choice.consequenceHint}
                        </span>
                      )}
                    </button>
          ...
          <div className="flex items-end gap-2">
            <textarea
              ...
              disabled={props.busy}
              rows={1}
              placeholder={props.actionPlaceholder}
              className="h-12 max-h-20 min-h-12 flex-1 resize-none overflow-y-auto rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm leading-5 text-[#eee8df] outline-none placeholder:text-white/25 focus:border-amber-300/50 disabled:opacity-50"
            />
            <button
              type="button"
              ...
              className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-amber-600 text-amber-50 disabled:opacity-35"
            >
              {props.busy ? (
          ...
          <p className="mt-1.5 px-1 text-[9px] text-white/30">
            {props.busy ? "故事正在生成并保存，请稍候…" : props.submitLabel}
          </p>
        </div>
      </section>
      ...
      {activePanel && (
        <div
          className="absolute inset-0 z-50 flex items-end bg-black/65"
        ...
        >
          <section
            ...
            onClick={(event) => event.stopPropagation()}
            className="max-h-[72%] w-full overflow-y-auto rounded-t-[2rem] border-t border-white/10 bg-[#1b1816] p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl"
          >
            <div className="mx-auto max-w-2xl">
              <div className="mb-5 flex items-center justify-between">
                <h2 className="text-base font-black">{activePanel.label}</h2>
                <button
                  type="button"
                  onClick={() => setActivePanelId(null)}
                  aria-label="关闭面板"
