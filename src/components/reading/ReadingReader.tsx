...
export default function ReadingReader({ userIdentityId, bookId, room, settings, character, relationship, worldBookContext, initialAnchorId, onClose }: ReadingReaderProps) {
  ...
  const discussionThreads = useMemo(() => {
    if (!room) return [];
    return listReadingDiscussions(room)
      .sort((left, right) => left.createdAt - right.createdAt)
      .map((discussion) => ({
        discussion,
        messages: discussion.id === discussionId ? discussionMessages : listDiscussionMessages(room, discussion.id),
      }));
  }, [discussionId, discussionMessages, room]);

  const getDiscussionChapterTitle = (chapterId?: string) => content?.chapters.find((chapterView) => chapterView.chapter.id === chapterId)?.chapter.title || chapterId || "未标注章节";
      ...
      {isSearchOpen && content && (
        <div className="absolute inset-0 z-40 flex bg-black/35" role="dialog" aria-modal="true" aria-label="搜索小说正文">
          <div className="mt-auto flex max-h-[88%] w-full flex-col rounded-t-[2rem] bg-[var(--surface)] text-[var(--text-primary)] shadow-2xl">
            <div className="flex items-center gap-2 border-b border-[var(--border)] p-4"><Search className="h-4 w-4" /><input autoFocus value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="搜索书中内容" className="h-10 min-w-0 flex-1 rounded-xl border border-[var(--border)] bg-transparent px-3 text-sm outline-none" /><button type="button" onClick={() => setIsSearchOpen(false)} aria-label="关闭搜索" className="flex h-8 w-8 items-center justify-center"><X className="h-4 w-4" /></button></div>
