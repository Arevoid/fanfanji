...
import { BookOpenText, ChevronLeft, Handshake, ListChecks, Save, ShieldAlert, Sparkles, UserRound, UsersRound } from "lucide-react";
...
export default function ReadingCoStoryView({ userIdentityId, book, initialCoStoryId, friends, settings, onClose }: ReadingCoStoryViewProps) {
  ...
  const turns = useMemo(() => story ? listReadingCoStoryTurns(story) : [], [story, stories]);
  const saves = useMemo(() => story ? listReadingCoStorySaves(story) : [], [story, stories]);
  ...
  const askFriend = async () => {
    ...
    try {
      const result = await generateReadingCoStoryAiAction({ story, mode, userRequest: request, settings: { apiKey: settings.apiKey || "", selectedModel: settings.selectedModel || "", apiEndpoint: settings.apiEndpoint, apiTemperature: settings.apiTemperature, streamCompatible: settings.streamCompatible } });
      setRequest("");
      refresh(result.story);
      setMessage(result.story.pendingApproval ? "AI 好友提出了重大行动，请由你确认。" : "AI 好友的行动已保存。");
    } catch (error) { setMessage(error instanceof Error ? error.message : "AI 好友行动失败"); }
    finally { setBusy(false); }
  };

  const resolveApproval = (approve: boolean) => {
    if (!story?.pendingApproval) return;
    try { const next = resolveReadingCoStoryApproval({ scope: story, actionId: story.pendingApproval.actionId, approve }); refresh(next); setMessage(approve ? "已接受 AI 好友行动。" : "已拒绝 AI 好友行动。"); }
    catch (error) { setMessage(error instanceof Error ? error.message : "处理行动失败"); }
  };

  const saveStory = () => {
    if (!story || !turns.length) return;
    try { createReadingCoStorySave({ scope: story, label: `第 ${turns.length} 回合` }); refresh(story); setMessage("已创建共同故事手动存档。"); }
    catch (error) { setMessage(error instanceof Error ? error.message : "存档失败"); }
  };
  ...
  const panels: ReadingStoryPanel[] = [
    { id: "status", label: "状态", icon: <ListChecks className="h-4 w-4" />, content: <div className={panelClass}><div className="grid grid-cols-2 gap-2">{[["故事状态", story.status === "completed" ? "已完成" : "进行中"], ["行动方", story.activeActor === "user" ? "你" : story.aiFriend.displayName], ["当前位置", story.currentLocation], ["当前时间", story.currentTime]].map(([label, value]) => <div key={label} className="rounded-2xl bg-white/5 p-3"><p className="text-[10px] text-white/35">{label}</p><p className="mt-1 font-bold text-white/80">{value}</p></div>)}</div><p className="text-white/35">共同故事自动保存，并按关系、角色和故事 ID 完全隔离。</p></div> },
  ...
  return <ReadingStoryPlayShell title={story.title} subtitle={`${story.aiFriend.displayName} · ${story.origin === "custom" ? "自建世界" : "共同穿书"}`} currentChapter={story.currentChapter} targetChapters={story.targetChapters} currentLocation={story.currentLocation} currentTime={story.currentTime} statusLabel={story.pendingApproval ? "等待你确认" : "自动保存"} choices={latestChoices} action={userAction} actionPlaceholder="输入你的行动；AI 好友不会替你选择……" submitLabel={settings ? "提交我的行动" : "请先配置 AI"} busy={busy} submitDisabled={!userAction.trim() || !settings || Boolean(story.pendingApproval)} notice={message} panels={panels} headerAction={<button type="button" onClick={saveStory} disabled={!turns.length} aria-label="保存共同故事" className="flex h-9 w-9 items-center justify-center rounded-full disabled:opacity-35"><Save className="h-4 w-4" /></button>} onActionChange={setUserAction} onSubmit={submitUserAction} onBack={onClose}>
    <section aria-label="共同故事正文" className="min-h-[45vh]">{turns.length ? turns.map((turn) => <article key={turn.turnId} className="mb-8 last:mb-0"><p className="whitespace-pre-wrap text-[15px] leading-8 text-[#eee8df]/90">{turn.narrative}</p>{turn.userAction && <p className="mt-3 border-l border-cyan-300/30 pl-4 text-xs leading-6 text-cyan-100/80">你的视角：{turn.userAction}</p>}{turn.aiAction && <p className="mt-3 border-l border-amber-300/30 pl-4 text-xs leading-6 text-amber-100/80">{story.aiFriend.displayName} 的视角：{turn.aiAction}</p>}</article>) : <div className="py-16 text-center"><Handshake className="mx-auto h-8 w-8 text-white/20" /><p className="mt-4 text-sm font-bold">共同故事还没有回合</p><p className="mt-2 text-xs text-white/40">先写下自己的行动，再让 AI 好友回应。</p></div>}</section>
    {story.pendingApproval && <section className="mt-6 rounded-3xl border border-amber-300/30 bg-amber-400/10 p-4"><div className="flex gap-3"><ShieldAlert className="h-5 w-5 shrink-0 text-amber-300" /><div><p className="text-sm font-bold">AI 好友提出重大行动</p><p className="mt-2 text-xs leading-5 text-white/70">{story.pendingApproval.action}</p><p className="mt-1 text-[10px] text-white/40">{story.pendingApproval.reason}</p></div></div><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => resolveApproval(false)} className="h-10 rounded-xl border border-white/10 text-xs font-bold">拒绝</button><button type="button" onClick={() => resolveApproval(true)} className="h-10 rounded-xl bg-amber-600 text-xs font-bold">接受</button></div></section>}
    <section className="mt-6 rounded-3xl border border-white/10 bg-white/[0.03] p-4"><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-amber-300" /><p className="text-xs font-bold">让 {story.aiFriend.displayName} 回应</p></div><div className="mt-3 grid grid-cols-3 gap-2">{(Object.keys(modeCopy) as Mode[]).map((item) => <button key={item} type="button" onClick={() => setMode(item)} className={`rounded-xl border p-2 text-[10px] font-bold ${mode === item ? "border-amber-300/50 bg-amber-400/10" : "border-white/10"}`}>{modeCopy[item]}</button>)}</div><textarea value={request} onChange={(event) => setRequest(event.target.value)} disabled={busy || Boolean(story.pendingApproval)} rows={2} placeholder="补充你想让 TA 关注的事情……" className="mt-3 w-full resize-none rounded-2xl border border-white/10 bg-white/5 p-3 text-xs outline-none" /><button type="button" disabled={!settings || busy || Boolean(story.pendingApproval)} onClick={askFriend} className="mt-2 flex h-10 w-full items-center justify-center gap-2 rounded-2xl bg-white/10 text-xs font-bold disabled:opacity-40"><Sparkles className="h-4 w-4" />{busy ? "正在等待 TA…" : settings ? modeCopy[mode] : "请先配置 AI"}</button></section>
