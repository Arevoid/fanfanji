...
import { Film, ImagePlus, MessageCircle, Play, Plus, Save, Subtitles, Trash2, Upload, Users, X } from "lucide-react";
...
export default function AppCinema({
  ...
  const createRoom = (relation: CharacterRelationship) => {
    ...
    if (persistStore(mergeStore(storeRef.current, { rooms: [room, ...storeRef.current.rooms] }))) {
      ...
      setNotice(`已与${characters.find((character) => character.id === relation.characterId)?.name || "角色"}建立观影房间`);
    }
  };
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[var(--app-bg)] text-[var(--text-primary)]">
      <header className="flex shrink-0 items-center justify-between border-b border-[var(--border)] px-4 py-2">
        <button type="button" onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--surface-raised)]" aria-label="关闭影视">
          <X className="h-4 w-4" />
        </button>
