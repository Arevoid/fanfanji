...
import { ArrowLeft, ArrowUp, Film, MessageCircle, Save, Send, Settings as SettingsIcon, Subtitles, Trash2, Upload, Users, X } from "lucide-react";
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[var(--app-bg)] text-[var(--text-primary)]">
      <header className="grid shrink-0 grid-cols-[40px_minmax(0,1fr)_40px] items-center border-b border-[var(--border)] px-4 py-2">
        <button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-full bg-[var(--surface-raised)]" aria-label="返回">
      ...
      <main className={`min-h-0 flex-1 p-4 ${selectedMedia ? "overflow-hidden" : "overflow-y-auto"}`}>
        {!selectedMedia ? (
          ...
          <section className="mx-auto flex h-full min-h-0 max-w-2xl flex-col gap-4">
            ...
            <div className="flex min-w-0 flex-nowrap items-center gap-1.5 overflow-x-auto pb-1"><button type="button" onClick={() => subtitleInputRef.current?.click()} className="inline-flex h-9 shrink-0 items-center gap-1 rounded-full border border-[var(--border)] px-2.5 text-[10px] font-bold" title={selectedMedia.subtitle ? "更换字幕" : "导入字幕"}><Subtitles className="h-4 w-4" />字幕</button><button type="button" onClick={() => { if (selectedRoom) setAutoReactionEnabled((value) => { const next = !value; persistStore(mergeStore(storeRef.current, { rooms: storeRef.current.rooms.map((room) => room.id === selectedRoom.id ? { ...room, autoReactionEnabled: next, updatedAt: Date.now() } : room) })); return next; }); }} disabled={!selectedRoom} className={`inline-flex h-9 shrink-0 items-center gap-1 rounded-full border px-2.5 text-[10px] font-bold ${autoReactionEnabled ? "border-emerald-300 text-emerald-600" : "border-[var(--border)] text-[var(--text-muted)]"}`}><MessageCircle className="h-4 w-4" />主动发言：{autoReactionEnabled ? "开" : "关"}</button><button type="button" onClick={() => { if (selectedRoom) setPlotContinuityEnabled((value) => { const next = !value; persistStore(mergeStore(storeRef.current, { rooms: storeRef.current.rooms.map((room) => room.id === selectedRoom.id ? { ...room, plotContinuityEnabled: next, updatedAt: Date.now() } : room) })); return next; }); }} disabled={!selectedRoom} className={`inline-flex h-9 shrink-0 items-center gap-1 rounded-full border px-2.5 text-[10px] font-bold ${plotContinuityEnabled ? "border-sky-300 text-sky-600" : "border-[var(--border)] text-[var(--text-muted)]"}`}>理解剧情：{plotContinuityEnabled ? "开" : "关"}</button><button type="button" onClick={() => setRoomPickerOpen(true)} className="inline-flex h-9 shrink-0 items-center gap-1 rounded-full bg-[var(--button-primary-bg)] px-2.5 text-[10px] font-bold text-[var(--button-primary-text)]"><Users className="h-4 w-4" />邀请</button><button type="button" onClick={() => void removeMedia(selectedMedia)} className="ml-auto flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-rose-200 text-rose-500" aria-label="删除这部影视" title="删除这部影视"><Trash2 className="h-4 w-4" /></button></div>
