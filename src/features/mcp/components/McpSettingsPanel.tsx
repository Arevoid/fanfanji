...
const RESEARCH_MCP_SERVER: McpServerConfig = {
  ...
  readOnlyOnly: true,
  discoveredTools: [
    { name: "web_search_exa", description: "联网搜索当前网页、新闻和事实，并返回可引用内容。", inputSchema: { type: "object" }, readOnly: true, enabled: true },
    { name: "web_fetch_exa", description: "读取指定网页的正文内容。", inputSchema: { type: "object" }, readOnly: true, enabled: true },
  ],
  updatedAt: Date.now(),
};
const HOTSEARCH_MCP_SERVER: McpServerConfig = {
  ...
  readOnlyOnly: true,
  discoveredTools: [
    { name: "query_hot_trending", description: "读取微博、知乎、B站、百度、头条、抖音、贴吧和掘金热榜。", inputSchema: { type: "object" }, readOnly: true, enabled: true },
    { name: "list_platforms", description: "列出支持的热榜平台。", inputSchema: { type: "object" }, readOnly: true, enabled: true },
  ],
  updatedAt: Date.now(),
};

const newServer = (): McpServerConfig => ({ id: crypto.randomUUID(), name: "", url: "", enabled: true, directFetch: false, readOnlyOnly: true, discoveredTools: [], updatedAt: Date.now() });
...
export function McpSettingsPanel() {
  ...
  const persist = (server: McpServerConfig) => {
    const next = { ...server, name: server.name.trim() || "未命名 MCP", url: server.url.trim(), updatedAt: Date.now() };
  ...
  const refreshTools = async (server: McpServerConfig) => {
    ...
    try {
      ...
      const next = { ...server, discoveredTools: tools, updatedAt: Date.now() };
      upsertMcpServer(next); setServers(loadMcpServers()); setExpanded(server.id); setMessage(`已发现 ${tools.length} 个工具；只有标记为只读的工具会启用。`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "MCP 发现失败。"); }
