...
export interface ReadingStoryPromptInput { story: ReadingStoryState; recentTurns: readonly ReadingStoryTurn[]; userAction: string; bookTitle?: string; }
export interface ReadingStoryPrompt { systemInstruction: string; message: string; }
...
export function buildReadingStoryPrompt(input: ReadingStoryPromptInput): ReadingStoryPrompt {
  const recent = input.recentTurns.slice(-4).map((turn) => `正文：${clean(turn.narrative, 5000)}\n地点：${clean(turn.currentLocation, 300)}\n时间：${clean(turn.currentTime, 100)}`).join("\n\n");
  ...
  const message = [
    ...
    `已知情报：${input.story.discoveredIntel.slice(-20).map((item) => clean(item, 400)).join("；") || "暂无"}`,
    `当前任务：${input.story.tasks.slice(-20).map((item) => clean(item, 400)).join("；") || "暂无"}`,
    "最近回合：",
    recent || "（故事刚开始）",
