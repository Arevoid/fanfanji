...
export function buildReadingCoStoryAiActionPrompt(input: { context: ReadingCoStoryAiContext; mode: "suggest" | "ask_opinion" | "low_risk_execute"; userRequest?: string }): { systemInstruction: string; message: string } {
  return {
    ...
    message: JSON.stringify({ mode: input.mode, persona: input.context.persona, role: input.context.role, story: input.context.currentStory, knownIntel: input.context.knownIntel, recentTurns: input.context.visibleRecentTurns, userRequest: input.userRequest || "请根据当前情境提出下一步行动" }),
  };
}
