...
import type { ReadingStoryState, ReadingStoryTurn } from "../../../domain/reading/storyTypes";
import { commitReadingStoryTurn, getReadingStory, listReadingStoryTurns, validateReadingStoryTurnResult, type ReadingStoryTurnResult } from "./readingStory";
...
export async function generateReadingStoryTurn(input: {
  ...
  requestId?: string;
  now?: number;
}): Promise<{ story: ReadingStoryState; result: ReadingStoryTurnResult; requestId?: string; attempts: number }> {
  if (!input.userAction.trim()) throw new Error("Please enter or choose an action first");
  if (!input.settings.apiKey?.trim() || !input.settings.selectedModel?.trim()) throw new Error("穿书 AI 配置不完整，请先设置 API Key 和模型");
  const scope = { userIdentityId: input.story.userIdentityId, storyId: input.story.storyId };
  ...
  if (requestId) {
    ...
    if (existing) {
      ...
      return { story: currentStory, result: resultFromTurn(existing), requestId, attempts: 0 };
    }
  }
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      if (error && typeof error === "object" && "code" in error && ((error as { code?: string }).code === "conflict" || (error as { code?: string }).code === "storage" || (error as { code?: string }).code === "missing")) throw error;
      lastError = error;
    }
  }
  throw lastError instanceof Error ? lastError : new Error("Story turn generation failed");
}
