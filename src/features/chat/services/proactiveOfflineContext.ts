...
const CHARACTER_PRESENT_NOW = /(?:我(?:已经|刚刚|现在)?(?:到了|到(?:你|你家|家里|门口|楼下|公司|学校)|在(?:你家|你那|门口|楼下|附近|外面|这里|你身边|你面前)|站在你面前|下来了|下楼了|下去接你了)|我(?:就)?在(?:门口|楼下|附近|外面|你身边|你面前)|我(?:就)?站在你面前|我到了|我在门口|(?:哥|哥哥|我)?(?:已经|刚刚|现在)?下(?:来了|楼了|去接你了)|(?:哥|哥哥)?在楼下(?:等你)?|(?:哥|哥哥)?到(?:楼下|门口)了|门口等你|我带着(?:东西|饭|礼物|电蚊拍|炖盅)?(?:来了|在门口)|(?:我)?(?:直接)?进(?:来了|去了)|进来吧|开门)/iu;
...
export function deriveProactiveOfflinePresenceEvidence(input: {
  ...
  const userRequestedOffline = recent.some((message) =>
    message.sender === "user" && isConcretePresenceMessage(visibleText(message), USER_OFFLINE_HANDOFF_INTENT));
