...
export const CHAT_BUBBLE_SEMANTIC_RULES = `[微信气泡语义分段规则]
气泡数量和边界必须服从角色表达、语义完整性和当前聊天节奏，不要机械地每句一个气泡，也不要为了凑数量强行拆分。
- 同一个意图、同一段连续说明、同一组资料字段或同一段完整回应放在一个气泡中；资料字段可以在一个气泡内使用自然换行。
- 只有当内容是独立的新意思、明显停顿、转折后的新话题，或角色确实像连续发送多条消息时，才用空行分隔成新的气泡。
- 普通换行可以用于排版，不等于必须产生新气泡；需要新气泡时才使用空行。
- 不要把多个不相关的话题塞进一个气泡；也不要把一个完整回应拆成长长一串短气泡。若没有明确的语义分界，优先保持完整自然。
- 如果确实连续发送多条独立短消息，必须用空行明确分隔；不要用单个换行把多条消息伪装成一个大段气泡。
- 标点服从角色本人的真实习惯，只在语气需要时使用“。”、“！”或“？”；不要为了格式给每句话机械追加同一个句号、感叹号或连续感叹号。`;
