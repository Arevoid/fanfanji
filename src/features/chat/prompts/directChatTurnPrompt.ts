...
2. Never invent payment artifacts (红包、转账、收款或付款请求) unless the user has sent that artifact in this turn or explicitly asks you to send one. A check-in phrase, relationship rule, promise, gift discussion, or affectionate message is not a payment request. Manual attachment actions are handled outside this prompt.
...
export function buildVoiceCallPrompts(callTopicShiftDetected: boolean, callMode: "voice" | "video" = "voice"): string[] {
  if (callMode === "video") {
    return [
      `[视频通话输出规则]
你正在和用户进行实时视频通话。请严格根据角色人设、世界观、双方关系、当前地点和已经确认的聊天事实来回应，不要使用固定模板。
每次回复必须包含以下两行，顺序固定，并且只调用一次生成：
[画面]|简短、可视化的角色动作/表情/环境变化；即使变化很小，也要给出当前镜头描述
[台词]|角色真正会说出口的话；如果角色此刻不想说话，也要用简短自然的回应表达
...
3. 若用户输入的是普通说话内容，按实时对话回应；不要把对话内容误写成用户画面。
4. 长期记忆只在用户明显换话题时按需参考；${callTopicShiftDetected ? "本轮可读取直接相关的长期事实。" : "本轮不要主动加载或强行套用旧记忆。"}
5. 画面描述和台词都必须遵循角色语言习惯、情绪、隐私边界和当前世界观。`,
    ];
  }
  return [
    `[语音电话输出规则]
