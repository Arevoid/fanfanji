...
export async function advanceCharacterPhone(input: { phone: CharacterPhoneRecord; character: Character; settings?: UserSettings; now?: number }): Promise<CharacterPhoneRecord> {
  const now = input.now ?? Date.now();
  const lastOpenedAt = input.phone.lastOpenedAt;
  const elapsed = lastOpenedAt ? now - lastOpenedAt : 0;
  const base = { ...input.phone, lastOpenedAt: now, updatedAt: now, scheduleItems: input.phone.scheduleItems ?? [], galleryItems: input.phone.galleryItems ?? [], contacts: input.phone.contacts ?? [], threadMessages: input.phone.threadMessages ?? [], posts: input.phone.posts ?? [] };
  ...
  if (input.settings?.apiKey && input.settings.selectedModel) {
    try {
      const response = await apiChat({
        message: `请为角色“${input.character.name}”的虚拟手机补充一小组生活痕迹。只返回 JSON，不要 Markdown：{"message":"角色本人发给某人的一句自然消息","searchQuery":"一条搜索词","searchTitle":"搜索结果标题","diaryTitle":"短日记标题","diaryBody":"80字以内的日记"}`,
        history: [],
