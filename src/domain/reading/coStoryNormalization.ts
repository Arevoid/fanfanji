...
export function normalizeReadingCoStoryStore(value: unknown): ReadingCoStoryStore {
  ...
  const turns = Array.isArray(source.turns)
    ? source.turns.filter((item): item is Record<string, unknown> => Boolean(item && typeof item === "object" && !Array.isArray(item))).filter(scopeValid).map((item) => ({
      ...
      requestId: text(item.requestId, 200) || undefined,
      ...
      userDiscoveredIntel: list(item.userDiscoveredIntel, 50),
      aiDiscoveredIntel: list(item.aiDiscoveredIntel, 50),
      currentLocation: text(item.currentLocation, 500) || undefined,
      currentTime: text(item.currentTime, 200) || undefined,
      chapterProgress: Math.max(0, Math.min(1, Number(item.chapterProgress) || 0)),
