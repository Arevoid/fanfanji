...
import { isWorldBookEntryVisible, type WorldBookReadContext } from "./worldBookVisibility";
...
export function getWorldBookLocationReferences(
  ...
  const context: WorldBookReadContext = readContext || { scenario: "chat", characterId };
  entries.filter((entry) => isWorldBookEntryVisible(entry, { ...context, characterId })).forEach((entry) => {
