...
assert.match(pipeline, /executeDirectReplyUseCase\(/, "normal direct reply must route through the application use case");
const useCase = readFileSync(new URL("../src/features/chat/services/directReplyUseCase.ts", import.meta.url), "utf8");
assert.match(useCase, /executeDirectReplyTurn\(/, "the use case must delegate the prepared turn to the thin turn executor");
