...
assert.ok(kinds.filter((kind) => kind === "virtual").length > 850, "NPC is the dominant long-run main-post author");
assert.ok(kinds.filter((kind) => kind === "relationship").length > 80 && kinds.filter((kind) => kind === "relationship").length < 120);
