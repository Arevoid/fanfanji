...
assert.match(hook, /enqueueConversationSummaryProjection/);
assert.match(hook, /buildCanonicalMemoryCommitSnapshot/);
assert.match(hook, /resolveDirectChatSummaryCutover/);
assert.match(hook, /writeSynchronousSummary/);
assert.match(hook, /canonicalSnapshotAvailable/);
assert.match(hook, /canonicalSnapshot\?\.activeClaims/);
assert.match(hook, /canonicalSnapshot\?\.sourceMessageIds/);
assert.match(hook, /if \(isAutomaticDirectChat && \(!cutoverDecision/);
assert.match(hook, /finalCutoverDecision\.canAdvanceCursor/);
assert.match(hook, /!isAutomaticDirectChat \|\| finalCutoverDecision\.requiresSynchronousFallback/);
assert.match(hook, /canonicalSnapshotAvailable/);
assert.match(hook, /manualMessagesOverride === undefined/);
