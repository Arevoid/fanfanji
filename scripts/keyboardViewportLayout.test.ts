...
assert.match(indexHtml, /<meta name="viewport"[^>]*interactive-widget=resizes-content/);
...
assert.match(viewport, /removeEventListener\("focusout"/);
assert.doesNotMatch(viewport, /localStorage/);

console.log("PASS root layout uses a single VisualViewport controller with rAF coalescing and cleanup");
