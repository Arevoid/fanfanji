...
const appearanceDraft = readFileSync(new URL("../src/features/settings/hooks/useSettingsAppearanceDraftState.ts", import.meta.url), "utf8");
...
assert.match(appearanceDraft, /desktopIconMode \|\| "dark"/);
assert.match(app, /desktopIconMode:\s*"dark"/);
...
assert.match(app, /desktopAppTextColor: "#000000"/);
assert.match(app, /--desktop-app-text-color: \$\{settings\.desktopAppTextColor \|\| "#000000"\}/);
assert.match(app, /desktop-app-label/);
assert.doesNotMatch(app, /font-extrabold:not\(\.desktop-app-label\)/);
