...
assert.equal(shadow.acceptedByTarget.truth, 0, "stable preference remains review-only in the additive policy");
assert.equal(shadow.acceptedByTarget.belief, 0, "missing epistemic status keeps belief review-only");
assert.equal(shadow.acceptedByTarget.event, 0, "missing epistemic status keeps event review-only");
assert.equal(shadow.acceptedByTarget.episodic, 0, "missing epistemic status keeps episodic review-only");
assert.equal(shadow.needsReviewByReason.missing_epistemic_status, 4);
assert.equal(shadow.needsReviewByReason.stable_preference_requires_review, 1);
assert.equal(shadow.needsReviewByReason.temporary_preference_not_durable, 1);
assert.equal(shadow.needsReviewByReason.relationship_signal_requires_review, 1);
assert.equal(shadow.rejectedByReason.scene_only_not_truth, 1);
assert.equal(shadow.rejectedByReason.subjective_not_objective_truth, 1, "Direct Chat subjective reflection cannot enter Truth");
