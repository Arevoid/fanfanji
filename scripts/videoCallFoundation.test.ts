...
assert.match(appChat, /onRegenerateCallTurn=\{regenerateCallTurn\}/);
assert.match(appChat, /const regenerateCallTurn =/);
...
assert.match(view, /onRegenerateCallTurn/);
assert.match(view, /重回/);
assert.match(view, /switchCamera/);
assert.match(view, /facingMode/);
assert.match(view, /onAccept/);
assert.match(view, /onReject/);
assert.match(view, /接听视频通话/);
...
assert.match(view, /inset-x-1 bottom-3 flex max-h-\[34%\]/);
assert.match(view, /self-end\" : \"self-start/);
assert.match(view, /text-left text-white\/90/);
