...
import { appendReadingDiscussionMessage, listDiscussionMessages, listReadingComments, startReadingDiscussion, createAiReadingComment, createUserReadingComment } from "../src/features/reading/coReading/readingCoReadingContent";
...
const userComment = createUserReadingComment({ scope: room, authorName: "我", kind: "book", body: "这本书的开场很有画面感。", targetParagraphAnchorId: anchor.id, now: 4 });
...
createAiReadingComment({ scope: room, authorName: character.name, targetParagraphAnchorId: anchor.id, textSnapshot: "第一段", body: "而且它和你的段评正好能接起来。", parentCommentId: userComment.id, now: 5.5 });
assert.equal(listReadingComments(room).at(-1)?.parentCommentId, userComment.id, "AI replies persist under the original room comment");
...
appendReadingDiscussionMessage({ scope: room, discussionId: discussion.id, author: "ai", authorName: character.name, body: "我更在意门口反复出现的声音。", now: 7 });
assert.equal(listDiscussionMessages(room, discussion.id).length, 2, "realtime discussion messages survive a repository reload");
