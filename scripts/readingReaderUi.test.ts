...
assert.match(appReading, /<ReadingReader userIdentityId=\{userIdentityId\} bookId=\{readingBookId\}/);
assert.match(appReading, /继续阅读 · \$\{selectedProgress\.percent\.toFixed\(1\)\}%/);
...
assert.match(reader, /scrollToAnchor\(target, "auto", progress\?\.characterOffset \|\| 0\)/);
assert.match(reader, /aria-label="打开目录"/);
assert.match(reader, /上一章[\s\S]*下一章/);
assert.doesNotMatch(reader, /overflow-x-auto|左右翻页|translateX/, "Round 5 remains a vertical continuous reader");
