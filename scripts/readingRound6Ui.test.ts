...
const library = readFileSync(new URL("../src/components/AppReading.tsx", import.meta.url), "utf8");
...
assert.match(reader, /createReadingAnnotation[\s\S]*kind: "highlight"[\s\S]*start: activeParagraph\.start/);
...
assert.match(reader, /fontAssetId: event\.target\.checked \? GLOBAL_FONT_ASSET_ID : undefined/);
assert.match(reader, /aria-label="阅读排版设置"/);
assert.match(library, /buildReadingArchive\(userIdentityId\)/);
assert.match(library, /restoreReadingArchive\(JSON\.parse\(await file\.text\(\)\), userIdentityId\)/);
assert.match(library, /已恢复 \$\{restored\.restoredBooks\} 本书，正文、进度和标注均已写回本地/);
