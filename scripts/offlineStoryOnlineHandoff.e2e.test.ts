...
const tests: Array<[string, () => void]> = [
  ...
  ["D collector keeps the user fact", () => assert.ok(collectOfflineHandoffContent(currentStory).includes("给了他一颗糖"))],
  ["E collector keeps the dinner promise", () => assert.ok(collectOfflineHandoffContent(currentStory).includes("晚上七点左右到家里吃饭"))],
  ["F collector preserves chronological order", () => assert.ok(collectOfflineHandoffContent(currentStory).indexOf(userEvent) < collectOfflineHandoffContent(currentStory).indexOf(characterEvent))],
  ...
  ["J handoff contains current story facts", () => assert.ok(handoff.content.includes(userEvent) && handoff.content.includes(characterEvent))],
  ...
  ["P the final mock API request contains the sugar fact", () => assert.ok(finalMockApiRequest.systemInstruction.includes("给了他一颗糖"))],
  ["Q the final mock API request contains the dinner promise", () => assert.ok(finalMockApiRequest.systemInstruction.includes("晚上七点左右到家里吃饭"))],
