...
function emptyResult(errors: string[], manifest?: MemoryAdmissionCampaignManifest): CampaignReviewResult {
  return {
    ...
    approvedWindowCount: Array.isArray(manifest?.approvedWindows) ? manifest.approvedWindows.length : 0,
    closedWindowCount: Array.isArray(manifest?.closedWindows) ? manifest.closedWindows.length : 0,
    windowCount: 0,
